...
namespace MvcApplication.Models
{
    ...
    public static class FoxRepo
    {
        ...
        public static Plants GetPlant(Plants plant = Plants.plant_none)
        {
            ...
            if (plant_name!=null)
            {
                if (Enum.IsDefined(typeof(Plants), plant_name))
                    pl = GetPlantFromNumber(plant_name);
                else
        ...
        /// <returns></returns>
        public static Plants GetPlantFromNumber(string plantNumber)
        {
            return (Plants) Enum.Parse(typeof(Plants), plantNumber, true);// ((Plants)int.Parse(prodNumber));
        }
        ...
        public static IEnumerable<T> GetTable<T>(bool useCache = false, bool isArchived = false, bool ShowDeleted = false, Plants plant = Plants.plant_none, string filter = "", params Object[] parameters) where T : global::System.Data.DataRow
        {
            ...
            // Обязательно коментить перед Commit
        ...
        static string Get_StartMs() {
            return FoxRepo.GetTable<CrystalDS.asparamRow>(filter: "WHERE identp == ?", parameters: new[] { "start_ms" }).Single().valp.Trim();
        }
        ...
        public static string GetPeriod(string fdate, string sdate, string indnumb)
        {
            ...
            switch (indnumb)
            {
                ...
                case "2":
                    ...
                    break;
                case "3":
                    str = "за " + DateTime.Parse(fdate).ToString("MMMM", System.Globalization.CultureInfo.CreateSpecificCulture("ru-RU")) + " " + DateTime.Parse(fdate).Year + " года";
                    break;
                case "4":
