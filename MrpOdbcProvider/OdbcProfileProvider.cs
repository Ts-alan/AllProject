...
(
  ...
  PropertyNames Text (255) NOT NULL,
  PropertyValuesString Text (255) NOT NULL,
  IsAnonymous YesNo, 
    ...
    public sealed class OdbcProfileProvider : ProfileProvider {
        ...
        public override SettingsPropertyValueCollection
            ...
            foreach (SettingsProperty prop in ppc) {
                ...
                pv.PropertyValue = GetPropValue(prop.Name, username, isAuthenticated);
                //switch (prop.Name) {
                //    case "StockSymbols":
        ...
        public override void SetPropertyValues(SettingsContext context,
            ...
            foreach (SettingsPropertyValue pv in ppvc) {
                SetPropValue(pv.Property.Name, username, isAuthenticated, (string)pv.PropertyValue);
        ...
        private string GetPropValue(string propname, string username, bool isAuthenticated) {
            string propValue = "";

            OdbcConnection conn = new OdbcConnection(connectionString);
            OdbcCommand cmd = new OdbcCommand("SELECT PropertyValuesString FROM Profiles " +
                  "WHERE PropertyNames = ? AND Username = ? AND ApplicationName = ? And IsAnonymous = ?", conn);
            cmd.Parameters.Add("@PropertyNames", OdbcType.VarChar, 255).Value = propname;
            cmd.Parameters.Add("@Username", OdbcType.VarChar, 255).Value = username;
            cmd.Parameters.Add("@ApplicationName", OdbcType.VarChar, 255).Value = ApplicationName;
            cmd.Parameters.Add("@IsAnonymous", OdbcType.Bit).Value = !isAuthenticated;
            ...
            try {
                ...
                propValue = (string)cmd.ExecuteScalar();
            }
            catch (OdbcException e) {
                if (WriteExceptionsToEventLog) {
                    WriteToEventLog(e, "GetPropValue");
                    throw new ProviderException(exceptionMessage);
                } else {
                    throw e;
                }
        ...
        // the call to SetPropertyValues.
        //
        private void SetPropValue(string propname, string username, bool isAuthenticated, string propValue) {
            if (propValue == null) { propValue = String.Empty; }

            OdbcConnection conn = new OdbcConnection(connectionString);

            OdbcCommand cmd = new OdbcCommand("DELETE FROM Profiles " +
            "WHERE PropertyNames = ? AND Username = ? AND ApplicationName = ? And IsAnonymous = ?", conn);
            cmd.Parameters.Add("@PropertyNames", OdbcType.VarChar, 255).Value = propname;
            cmd.Parameters.Add("@Username", OdbcType.VarChar, 255).Value = username;
            cmd.Parameters.Add("@ApplicationName", OdbcType.VarChar, 255).Value = ApplicationName;
            ...
            OdbcCommand cmd2 = new OdbcCommand("INSERT INTO Profiles (Username, " +
                    "ApplicationName, LastActivityDate, LastUpdatedDate, " +
                    "IsAnonymous, PropertyNames, PropertyValuesString) Values(?, ?, ?, ?, ?, ?, ?)", conn);

            cmd2.Parameters.Add("@Username", OdbcType.VarChar, 255).Value = username;
            cmd2.Parameters.Add("@ApplicationName", OdbcType.VarChar, 255).Value = ApplicationName;
            cmd2.Parameters.Add("@LastActivityDate", OdbcType.DateTime).Value = DateTime.Now;
            cmd2.Parameters.Add("@LastUpdatedDate", OdbcType.VarChar).Value = DateTime.Now;
            cmd2.Parameters.Add("@IsAnonymous", OdbcType.Bit).Value = !isAuthenticated;
            cmd2.Parameters.Add("@PropertyNames", OdbcType.VarChar, 255).Value = propname;
            cmd2.Parameters.Add("@PropertyValuesString", OdbcType.VarChar, 255).Value = propValue;
            ...
            try {
                ...
                if (WriteExceptionsToEventLog) {
                    WriteToEventLog(e, "SetPropValue");
                    throw new ProviderException(exceptionMessage);
                } else {
                    throw e;
                }
        ...
        private bool DeleteProfile(string username, OdbcConnection conn, OdbcTransaction tran) {
            ...
            OdbcCommand cmd1 = new OdbcCommand("DELETE * FROM Profiles WHERE Username = ?", conn);
            cmd1.Parameters.Add("@Username", OdbcType.VarChar, 255).Value = username;
