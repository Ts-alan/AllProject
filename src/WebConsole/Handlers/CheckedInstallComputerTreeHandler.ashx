...
public class CheckedInstallComputerTreeHandler : IHttpHandler
{
    ...
    public void ProcessRequest(HttpContext context)
    {
        ...
        tree.Add(TreeJSONEntityConverter.ConvertToTreeNodeJsonEntity(
            new Group(0, Resources.Resource.Computers, Resources.Resource.Computers, null), false, true, true));

        foreach (String item in provider.GetComputerNames())
        {
            tree[0].Children.Add(TreeJSONEntityConverter.ConvertToTreeNodeJsonEntity(item, false,true, false, true)); 
        }
