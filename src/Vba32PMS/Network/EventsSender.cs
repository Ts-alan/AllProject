...
namespace Vba32.ControlCenter.PeriodicalMaintenanceService.Network
{
    public static class EventsSender
    {
        ...
        {
            Debug.WriteLine("EventsSender.ConnectSocket():: оннектимс€ к удаленному серверу");
            Socket s = null;
            try
            {
        ...
        {
            Debug.WriteLine("EventsSender.SocketSendReceive()::ѕосылаем пакет");
            ...
            Debug.WriteLine("–азмер отсылаемого сообщени€: "+mes.Length);
            Debug.WriteLine("–азмер отсылаемого пакета: "+bytesSent.Length);
