...
namespace Vba32.ControlCenter.PeriodicalMaintenanceService.Network
{
    public static class EventsSender
    {
        ...
        {
            Logger.Debug("EventsSender.ConnectSocket():: оннектимс€ к удаленному серверу");
            Socket s = null;
            try
            {
        ...
        {
            Logger.Debug("EventsSender.SocketSendReceive()::ѕосылаем пакет");
            ...
            Logger.Debug("–азмер отсылаемого сообщени€: "+mes.Length);
            Logger.Debug("–азмер отсылаемого пакета: "+bytesSent.Length);
